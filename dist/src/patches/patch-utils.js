...
function generateTimestamp() {
    ...
    const countryCode = tzInfo && Array.isArray(tzInfo.countries) && tzInfo.countries.length > 0
        ? tzInfo.countries[0]
        : tzInfo.country ?? undefined;
    const countryInfo = countryCode ? ct.getCountry(countryCode) : null;
    // Safely read continent code and map to readable name
    const continentCode = countryInfo && typeof countryInfo.continent === "string"
        ? countryInfo.continent
        : "UNK";
    const continent = continentMap[continentCode] ?? "Unknown";
    // Safely read country name
    const country = countryInfo && typeof countryInfo.name === "string"
        ? countryInfo.name
        : "Unknown";
    return {
...
function ensurePatchFolder(patchName) {
...
function writeTxtLedger(patchRoot, opts, ts) {
...
function writeMdNarrative(patchRoot, opts, ts) {
...
function writeJsonMetadata(patchRoot, opts, ts) {
...
export async function createPatch(opts) {
