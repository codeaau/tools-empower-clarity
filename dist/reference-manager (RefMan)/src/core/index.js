...
export function tagAdd(refId, tag) {
    return {
        eventId: generateId("EVT"),
        refId,
        type: "TagAdd",
...
export function tagRemove(refId, tag) {
    return {
        eventId: generateId("EVT"),
        refId,
        type: "TagRemove",
...
export function updateMeta(refId, patch) {
    // Defensive: do not allow empty patch
    const hasKeys = Object.keys(patch).length > 0;
    if (!hasKeys)
        throw new Error("updateMeta requires at least one field");
    return {
        eventId: generateId("EVT"),
        refId,
        type: "UpdateMeta",
...
export function relate(refId, targetId, relation) {
    return {
        eventId: generateId("EVT"),
        refId,
        type: "Relate",
...
export function project(events) {
    const state = { ref: undefined, relations: [] };
    ...
    for (const ev of sorted) {
        switch (ev.type) {
            ...
            case "TagAdd": {
                ...
                if (!state.ref.tags.includes(t))
                    state.ref.tags.push(t);
                break;
            }
            case "TagRemove": {
                if (!state.ref)
            ...
            case "UpdateMeta": {
                ...
                if (title !== undefined)
                    state.ref.title = title;
                if (authors !== undefined)
                    state.ref.authors = authors;
                if (type !== undefined)
                    state.ref.type = type;
                if (source !== undefined)
