...
function listFromString(input, sep = ",") {
    if (!input)
        return [];
    return input
        .split(sep)
...
function formatListForLog(items) {
    return items.length ? items.map((i) => `- ${i}`).join("\n") : "-";
}
// ---------- Logging / append ----------
function appendLog(opts) {
    ensureHeader();
    const now = new Date();
...
function searchSessions(keyword) {
    ...
    sessions.forEach((s, idx) => {
        ...
        if (haystack.includes(keyword.toLowerCase())) {
            count++;
            console.log(`[${idx + 1}] ${s.date} | Goal: ${s.goal} | Duration: ${s.duration} | Tags: ${s.tags}`);
        }
    });
    if (count === 0) {
...
function showDetail(sessionNumber) {
    ...
    const formatList = (items) => items.length ? items.map((it) => `- ${it}`).join("\n") : "-";
    console.log(`\n=== Session #${sessionNumber} ===`);
